	...
	for(var i=0;i<=length;i++){
		...
		if (i==0){
			...
			for(j=0;j<(t-b);j++){
				...
				else if(resultset[i-1]['DAY'+currentdate2] =='PL' || resultset[i-1]['DAY'+currentdate2] =='Comp off' || resultset[i-1]['DAY'+currentdate2] =='Comp-Off' || resultset[i-1]['DAY'+currentdate2] =='Location Holiday')
				{
					shiftCell.setAttribute("style", "background-color: #FE642E;");
				}
				shiftCell.innerHTML=resultset[i-1]['DAY'+currentdate2];
